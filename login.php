...
if ( ! empty( $_POST ) ) {
    if ( isset( $_POST['username'] ) && isset( $_POST['password'] ) ) {
        ...
        $query = "SELECT * FROM employee WHERE User_Name = '" . $_POST['username']."'";
        echo $query;
        ...
        if ( password_verify( $_POST['password'], password_hash($row['Password'], PASSWORD_DEFAULT) ) ) {
          ...
          $_SESSION['login_email'] = $row["E_Mail"];
          $_SESSION['login_user'] = $row["First_Name"] . " " . $row["Middle_Initial"] . " " . $row["Last_Name"];
        }      
